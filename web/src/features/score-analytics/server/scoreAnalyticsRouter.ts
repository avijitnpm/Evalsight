...
export const createScoreAnalyticsRouter = ({
  adaptiveFinalThreshold = 100_000,
  samplingThreshold = 100_000,
  targetSampleSize = 100_000,
}: {
  adaptiveFinalThreshold?: number;
  ...
  createTRPCRouter({
    ...
    estimateScoreComparisonSize: protectedProjectProcedure
      ...
      .query(async ({ input }) => {
        ...
        const willSample =
          estimates.score1Count > samplingThreshold ||
          estimates.score2Count > samplingThreshold;

        const willSkipFinal =
          estimates.score1Count >= adaptiveFinalThreshold ||
          estimates.score2Count >= adaptiveFinalThreshold;
    ...
    getScoreComparisonAnalytics: protectedProjectProcedure
      ...
      .query(async ({ input }) => {
        ...
        // Calculate rate based on larger table to target the same sample size in both tables
        const maxCount = Math.max(estimates.score1Count, estimates.score2Count);
        const samplingRate = shouldSample
          ? Math.min(1.0, targetSampleSize / maxCount)
          : 1.0;
...
export const scoreAnalyticsRouter = createScoreAnalyticsRouter();
